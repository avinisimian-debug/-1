...
import { prepareAudioForWhisper } from "@/lib/prepare-audio";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ACCEPTED_EXTENSIONS.includes(extension)) {
      return NextResponse.json(
        { error: "Unsupported file type. Use MP3, WAV, MP4, or M4A." },
        { status: 400 },
      );
    }
    ...
    const { file: audioFile } =
      file.size > WHISPER_MAX_BYTES
        ? await prepareAudioForWhisper(file)
        : { file };
    ...
    if (!rawAnalysis) {
      return NextResponse.json(
        { error: "GPT-4o did not return an analysis." },
        { status: 500 },
      );
    }
    ...
    const result: TranscriptionResult = {
      ...
      transcript,
      ...(isPro && analysis.chapters?.length
